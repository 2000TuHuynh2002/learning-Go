...
func main() {
	...
	// number of elements). An uninitialized slice equals to nil and has length 0.
	var s []string
	...
	// In addition to these basic operations, slices support several more that make
	// them richer than arrays. One is the builtin `append`, which returns a slice
	...
	// Slices support a “slice” operator with the syntax `slice[low:high]`. For example,
	...
	// Slices can be composed into multi-dimensional data structures. The length of
	// the inner slices can vary, unlike with multi-dimensional arrays.
	twoD := make([][]int, 3)
	for i := range 3 {
		innerLen := i + 1
