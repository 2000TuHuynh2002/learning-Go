...
func main() {
	...
	// Many methods are available on these structs. Here’s a match test like we saw
	// earlier.
	...
	// This also finds the first match but returns the start and end indexes for the
	...
	// and the submatches within those matches. For example this will return information
	...
	// We can also provide `[]byte` arguments and drop `String` from the function
	// name.
	fmt.Println(r.Match([]byte("peach")))

	// When creating global variables with regular expressions you can use the `MustCompile`
	// variation of `Compile`. `MustCompile` panics instead of returning an error,
	// which makes it safer to use for global variables.
	r = regexp.MustCompile("p([a-z]+)ch")
	fmt.Println("regexp:", r)

	// The `regexp` package can also be used to replace subsets of strings with other
	// values.
