...
// and `zeroptr`. `zeroval` has an `int` parameter, so arguments will be passed
// to it by value. `zeroval` will get a copy of `ival` distinct from the one in
...
// from its memory address to the current value at that address. Assigning a value
