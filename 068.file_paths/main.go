...
func main() {
	// `Join` should be used to construct paths in a portable way. It takes any number
	// of arguments and constructs a hierarchical path from them.
	p := filepath.Join("dir1", "dir2", "filename")
	fmt.Println("p:", p)

	// You should always use `Join` instead of concatenating `/s` or `\s` manually.
	// In addition to providing portability, `Join` will also normalize paths by removing
	...
	// `Rel` finds a relative path between a base and a target. It returns an error
