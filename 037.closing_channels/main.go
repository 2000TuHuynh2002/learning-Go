...
// the `main()` goroutine to a worker goroutine. When we have no more jobs for the
// worker we’ll `close` the `jobs` channel.
func main() {
	...
	// Reading from a closed channel succeeds immediately, returning the zero value
	// of the underlying type. The optional second return value is `true` if the value
	// received was delivered by a successful send operation to the channel, or `false`
