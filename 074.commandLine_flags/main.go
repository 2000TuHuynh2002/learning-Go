...
func main() {
	...
	// Here we declare a string flag `word` with a default value "foo" and a short
	// description. This `flag.String` function returns a string pointer (not a string
	// value); we’ll see how to use this pointer below.
	wordPtr := flag.String("word", "foo", "a string")

	// This declares `numb` and `fork` flags, using a similar approach to the `word`
	// flag.
	...
	// Note that we need to dereference the pointers with e.g. `*wordPtr` to get the
