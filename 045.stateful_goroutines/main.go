...
// write that state, other goroutines will send messages to the owning goroutine
// and receive corresponding replies. These `readOp` and `writeOp` structs encapsulate
...
func main() {
	...
	// over the `reads` channel, and then receiving the result over the provided `resp`
	// channel.
	for range 100 {
		go func() {
			for {
